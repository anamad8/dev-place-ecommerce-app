...
import style from'./IconLogin.module.css';

function IconLogin() {
    
    const [userOpen, setUserOpen] = useState(false);

    const usuario = localStorage.getItem("name")[0].toUpperCase()
        ...
        <>
            ...
            >
                <div className={style.buttonCart}>
                    {!userOpen ? (
                        <p className={style.letraUsur}>{usuario}</p>
                    ) : (
                        <svg
                        width="20"
                        height="20"
                ...
                <div className={style.cart}>
                    {
                        admin === "true" ?
                        <>
                            <Link to="/CargarProductos">Cargar Productos</Link>
                            <Link to="/EditarProductos">Editar Productos</Link>
                        </>
                        :
                        <p></p>
                    }
